...
async function getOrCreateHostIds(hostnames) {
  ...
  if (uncachedHosts.length > 0) {
    try {
      ...
      await pool.query(
        'INSERT IGNORE INTO hosts (hostname) VALUES ?',
      ...
      const [hosts] = await pool.query(
        'SELECT id, hostname FROM hosts WHERE hostname IN (?)',
        [uncachedHosts]
      );
      ...
      if (hosts.length > 0) {
        console.log(`[HostService] Cached ${hosts.length} new hosts (total cached: ${hostCache.size})`);
      }
    } catch (error) {
